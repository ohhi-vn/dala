defmodule Dala.RendererTest do
  ...
  defmodule MockNIF do
    ...
    def set_taps(t), do: Agent.update(__MODULE__, fn s -> %{s | calls: [{:set_taps, [t]} | s.calls]} end)
    def apply_patches(b), do: Agent.update(__MODULE__, fn s -> %{s | calls: [{:apply_patches, [b]} | s.calls]} end)
