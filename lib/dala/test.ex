defmodule Dala.Test do
  ...
  def tap(node, tag) do
    :rpc.call(node, Process, :send, [:dala_screen, {:tap, tag}, []])
    :ok
  end
  ...
  @spec back(node()) :: :ok
  def back(node) do
    :rpc.call(node, Process, :send, [:dala_screen, {:dala, :back}, []])
    :ok
  end
  ...
  def select(node, list_id, index) when is_atom(list_id) and is_integer(index) do
    :rpc.call(node, Process, :send, [:dala_screen, {:select, list_id, index}, []])
    :ok
  end
  ...
  def send_message(node, message) do
    :rpc.call(node, Process, :send, [:dala_screen, message, []])
    :ok
  end

  # ── Native UI — unmodified app test harness ─────────────────────────────────
  #
  # These functions drive the native UI of any app — not just Dala-rendered ones.
  # They call dala_nif directly via RPC and do not require a dala screen process.
  ...
  def ui_tree(node) do
    :rpc.call(node, Dala.Platform.Native, :ui_tree, [])
  end
  ...
  def view_tree(node) do
    case :rpc.call(node, Dala.Platform.Native, :ui_view_tree, []) do
  ...
  def ax_action(node, match, action) do
    :rpc.call(node, Dala.Platform.Native, :ax_action, [match, action])
  end
  ...
  def ax_action_at_xy(node, x, y, action) do
    :rpc.call(node, Dala.Platform.Native, :ax_action_at_xy, [x * 1.0, y * 1.0, action])
  end
  ...
  def screen_info(node) do
    :rpc.call(node, Dala.Platform.Native, :screen_info, [])
  end
  ...
  def tap_xy(node, x, y) do
    :rpc.call(node, Dala.Platform.Native, :tap_xy, [x * 1.0, y * 1.0])
  end
  ...
  def long_press_xy(node, x, y, duration_ms \\ 800) do
    :rpc.call(node, Dala.Platform.Native, :long_press_xy, [x * 1.0, y * 1.0, duration_ms])
  end
  ...
  def swipe(node, x1, y1, x2, y2) do
    :rpc.call(node, Dala.Platform.Native, :swipe_xy, [x1 * 1.0, y1 * 1.0, x2 * 1.0, y2 * 1.0])
  end
  ...
  # They call Dala.WebView.interact/2 via RPC.
  ...
  defp nav(node, action) do
    :rpc.call(node, GenServer, :call, [:dala_screen, {:navigate, action}])
    :ok
  end

  defp rpc(node, call) do
    :rpc.call(node, GenServer, :call, [:dala_screen, call])
  end
