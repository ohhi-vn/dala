defmodule Dala.Connectivity.Dist do
        ...
        case Dala.Platform.Native.platform() do
          ...
          :android ->
            ...
            if node != base_node do
              Dala.Platform.Native.log("Dala.Dist: node suffix applied — #{base_node} → #{node}")
            end
  ...
  defp start_after(node, cookie, delay, dist_port) do
    ...
    #
    # When `mix dala.connect` is running, it sets up `adb reverse tcp:4369 tcp:4369`
    ...
    Dala.Platform.Native.log("Dala.Dist: waiting for EPMD (adb reverse tcp:4369 tcp:4369)...")

    case wait_for_epmd(10_000) do
      :ready ->
        Dala.Platform.Native.log("Dala.Dist: EPMD reachable, starting dist")
        ...
        Dala.Platform.Native.log("Dala.Dist: result=#{inspect(result)}")

        case result do
          {:ok, _} ->
            Node.set_cookie(cookie)
            Dala.Platform.Native.log("Dala.Dist: distribution started")
