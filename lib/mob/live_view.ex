defmodule Mob.LiveView do
  ...
  **LiveView works in the browser but not in the WebView**
  ...
  In your `lib/my_app/app.ex` (generated by `mix mob.new --liveview`):

      def start do
        # Suppress esbuild/tailwind warnings on-device
        Mob.LiveView.suppress_dev_tool_warnings()
        ...
      end
  """
  def suppress_dev_tool_warnings do
    # Only suppress on mobile platforms where these tools can't possibly run
    if :mob_nif.platform() in [:android, :ios] do
  ...
  Mob LiveView apps on the same device. The hash maps the app name to a port
  ...
  Or set the `MOB_LIVEVIEW_PORT` environment variable to force a specific port.
  """
  ...
  # 1. Environment variable MOB_LIVEVIEW_PORT (for runtime override)
  # 2. Application config :mob, :liveview_port
  # 3. Hash-based default (4200-4999 range)
  defp liveview_port do
    case System.get_env("MOB_LIVEVIEW_PORT") do
      nil ->
        case Application.get_env(:mob, :liveview_port) do
          nil -> default_port()
          port -> port
        end
  ...
  # This avoids collisions when multiple Mob LV apps are installed on the same device.
