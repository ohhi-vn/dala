// DalaRootView.swift — SwiftUI entry point. Observes DalaViewModel and renders the
// node tree pushed by BEAM NIFs via DalaViewModel.setRoot().
...
public typealias DalaNativeSend = (_ event: String, _ payload: [String: Any]) -> Void
public typealias DalaNativeViewFactory = (_ props: [String: Any], _ send: @escaping DalaNativeSend) ->
    AnyView

public final class DalaNativeViewRegistry {
    public static let shared = DalaNativeViewRegistry()
    private var factories: [String: DalaNativeViewFactory] = [:]

    public func register(_ name: String, factory: @escaping DalaNativeViewFactory) {
        factories[name] = factory
    }

    func view(for node: DalaNode) -> AnyView? {
        guard let name = node.nativeViewModule,
            let factory = factories[name],
            let props = node.nativeViewProps as? [String: Any]
        else { return nil }
        let handle = node.nativeViewHandle
        let send: DalaNativeSend = { event, payload in
            if let data = try? JSONSerialization.data(withJSONObject: payload),
                let json = String(data: data, encoding: .utf8)
            {
                dala_send_component_event(handle, event, json)
            }
        }
...
extension View {
    ...
    /// Apply Dala gesture handlers from a node — long press, double tap, swipe.
    ...
    @ViewBuilder
    func dalaGestures(_ node: DalaNode) -> some View {
        let hasSwipe =
...
// Allow DalaNode to be used as ForEach identity (NSObject provides hash/isEqual).
extension DalaNode: Identifiable {
...
extension DalaNode: Equatable {
    public static func == (lhs: DalaNode, rhs: DalaNode) -> Bool {
...
extension DalaNode {
    var childNodes: [DalaNode] {
        children.compactMap { $0 as? DalaNode }
    }
    ...
    var paddingEdgeInsets: EdgeInsets {
        // Use objc associated objects for caching since we can't modify DalaNode directly
    ...
    var body: some View {
        Group {
            ...
            case .scroll:
                ...
                .modifier(DalaScrollObserverGate(node: node, isHorizontal: isHorizontal))
            ...
            case .tabBar:
                ...
                DalaTabView(node: node, tabs: tabs)
            ...
            case .cameraPreview:
                DalaCameraPreviewView(facing: node.cameraFacing)
                    .ifLet(node.fixedWidth > 0 ? node.fixedWidth : nil) { v, w in
                        v.frame(width: CGFloat(w))
                    }
                    .ifLet(node.fixedHeight > 0 ? node.fixedHeight : nil) { v, h in
            ...
            case .nativeView:
                if let view = DalaNativeViewRegistry.shared.view(for: node) {
...
private struct DalaTabView: View {
    let node: DalaNode
    let tabs: [[String: Any]]
...
private struct DalaVideoPlayer: UIViewControllerRepresentable {
    let src: String
    let autoplay: Bool
    let loop: Bool
    let controls: Bool
...
private struct DalaCameraPreviewView: UIViewRepresentable {
    let facing: String
    ...
    class Coordinator: NSObject {
        ...
        func startObserving(view: CameraPreviewUIView) {
            ...
            observer = NotificationCenter.default.addObserver(
                forName: .dalaCameraSessionChanged,
                object: nil,
                queue: .main
            ) { [weak self] _ in
...
extension Notification.Name {
    static let dalaCameraSessionChanged = Notification.Name("DalaCameraSessionChanged")
}

// ── WebView ───────────────────────────────────────────────────────────────────

private let kDalaJsShimSwift =
    "(function(){" + "if(window.dala)return;" + "var _h=[];" + "window.dala={"
    + "send:function(d){window.webkit.messageHandlers.dala.postMessage(JSON.stringify(d));},"
...
private struct DalaWebView: View {
    let node: DalaNode
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            DalaWKWebView(node: node)
        }
    }
}

private struct DalaWKWebView: UIViewRepresentable {
    let node: DalaNode
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        config.userContentController.add(context.coordinator, name: "dala")
        let shim = WKUserScript(
            source: kDalaJsShimSwift,
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        var node: DalaNode
        init(node: DalaNode) { self.node = node }

        // JS → Elixir: window.dala.send(data) arrives here.
        // Delegates to dala_deliver_webview_message() in dala_nif.m.
        func userContentController(
            _ ucc: WKUserContentController,
            didReceive message: WKScriptMessage
        ) {
            guard message.name == "dala", let json = message.body as? String else { return }
            dala_deliver_webview_message(json)
        }
        ...
        if let error = error {
            ...
            if let stringResult = result as? String {
                ...
                json = "{\"result\": null}"\n            }\n            dala_deliver_webview_eval_result(json)\n        }\n        }
        ...
        func webView(
            ...
            if allowList.contains(where: { url.hasPrefix($0) }) {
                ...
                dala_deliver_webview_blocked(url)
            }
        }
    }
}
...
private struct DalaTextField: View {
    let node: DalaNode
    ...
    init(node: DalaNode, placeholder: String, initialText: String) {
        self.node = node
    ...
    var body: some View {
        TextField(placeholder, text: $text)
            ...
            // the bound value via Dala.Socket.assign without user input.
            ...
            // focused. Without the `if isFocused` guard, every DalaTextField
...
private struct DalaToggle: View {
    let node: DalaNode
...
private struct DalaSlider: View {
    let node: DalaNode
...
public struct DalaRootView: View {
    @ObservedObject var model = DalaViewModel.shared
    @Environment(\.colorScheme) private var colorScheme
    @State private var currentRoot: DalaNode? = nil
    ...
    public var body: some View {
        ...
        .onChange(of: model.rootVersion) {
            ...
            // Format: [DalaNav] transition=<push|pop|reset|none> navVersion=<n>
            if t != "none" {
                NSLog("[DalaNav] transition=%@ navVersion=%d", t, newNavVersion)
            }
        ...
        .onChange(of: colorScheme) { _, newScheme in
            dala_notify_color_scheme(newScheme == .dark ? "dark" : "light")
        }
    }
...
//
// DalaScrollObserver wires SwiftUI's scroll-geometry observer to DalaNode's
...
// DalaScrollObserverGate applies the iOS 18+ observer when available and
...
struct DalaScrollObserverGate: ViewModifier {
    let node: DalaNode
    let isHorizontal: Bool

    func body(content: Content) -> some View {
        if #available(iOS 18.0, *) {
            content.modifier(DalaScrollObserver(node: node, isHorizontal: isHorizontal))
        } else {
            content
        }
    }
...
// DalaScrollObserver wires SwiftUI's onScrollGeometryChange (iOS 18+) to the
// DalaNode closures populated by dala_nif.m. Throttling and delta-thresholding
// happen native-side in dala_send_scroll, so this modifier just forwards every
// geometry change. End-of-scroll is detected by a debounced "no motion for N
// ms" timer.
@available(iOS 18.0, *)
struct DalaScrollObserver: ViewModifier {
    let node: DalaNode
    let isHorizontal: Bool
