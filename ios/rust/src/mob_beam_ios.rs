// mob_beam_ios.rs — Complete Mob BEAM launcher for iOS.
// Full Rust port of mob_beam.m with erl_start binding
...
// mob_set_startup_phase/error are implemented in mob_nif.rs
extern "C" {
    fn mob_set_startup_phase(phase: *const c_char);
    fn mob_set_startup_error(error: *const c_char);
}

// Logging macro for iOS
macro_rules! logi {
    ($fmt:expr $(, $args:expr)*) => {
        println!("[MobBeam] {}", format!($fmt $(, $args)*));
    };
}

// Resolve the simulator's OTP runtime root at startup
fn resolve_sim_otp_root() -> PathBuf {
    if let Ok(env) = std::env::var("MOB_SIM_RUNTIME_DIR") {
        if !env.is_empty() {
...
#[no_mangle]
pub extern "C" fn mob_init_ui() {
    println!("[MobBeam] mob_init_ui: SwiftUI mode ready");
}

// Write diagnostic file
fn mob_write_diag(docs_dir: &str, name: &str, info: &str) {
...
#[cfg(all(feature = "mob_bundle_otp", not(target_os = "simulator")))]
...
pub extern "C" fn mob_start_beam(app_module: *const c_char) {
    ...
    let otp_root = {
        ...
        if let Ok(bundle_path) = std::env::var("MOB_OTP_BUNDLE_PATH") {
    ...
    #[cfg(not(feature = "mob_bundle_otp"))]
    ...
    // Runtime override from beams_dir/mob_beam_flags
    let flags_path = beams_dir.join("mob_beam_flags");
    ...
    unsafe {
        ...
        logi!(
            "mob_start_beam: calling erl_start with {} arguments",
            args.len() - 1
        );
...
#[no_mangle]
pub extern "C" fn mob_send_push_token(_hex_token: *const c_char) {
    // Implementation for push token forwarding
}

#[no_mangle]
pub extern "C" fn mob_set_launch_notification_json(_json: *const c_char) {
