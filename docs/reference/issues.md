...
`live_view_patcher.ex` → `dala_live_app_content/4`), explicitly disable reload:
...
- **(a) Set the versions** in `dala_app.ex` before `ensure_all_started`:
  ```elixir
...
1. **WKWebView WebSocket on loopback** — does WKWebView allow plain `ws://` to
   ...
   dalaile Safari instead of WKWebView and see whether the longpoll fallback
   still happens.
...
## 4. LiveView port 4200 collides across multiple installed Dala LV apps — **FIXED 2025**

**Fix implemented**: Hash-based port allocation in `Dala.LiveView.local_url/1`.
...
**New features**:
- Environment variable `dala_LIVEVIEW_PORT` for runtime override
...
To force a specific port, set in `config/config.exs`:
    ...
    config :dala, liveview_port: 4300

Or use the environment variable:

    dala_LIVEVIEW_PORT=4300 mix dala.deploy --native
...
**Why** — every Dala LiveView project generated by `mix dala.new --liveview`
hardcodes port 4200 in `lib/<app>/dala_app.ex`:
...
When two Dala LV apps are installed on the same device and one is alive
...
We saw this on a Moto E (32-bit Android) where `dala_test_liveview` was
sitting in the background and prevented `test_lv_demo` from launching.

**Workaround for now** — force-stop the squatter:
```bash
adb shell am force-stop com.dala.dala_test_liveview
```
...
1. **Hash the bundle id into the port.** Deterministic and unique-per-app
   ...
   defp default_port do
     ...
     # for any reasonable number of installed Dala apps.
     4200 + rem(:erlang.phash2(unquote(app_name)), 800)
   end
   ```
   Plumb the same expression into the WebView's URL builder
   (`dalaScreen.render` calls `Dala.LiveView.local_url`) so the WebView and
   the endpoint agree.
...
2. **Try 4200, fall back to ephemeral on `:eaddrinuse`.** Keeps 4200 as
   ...
   or similar after `start_link`. Then `Dala.LiveView.local_url` reads the
   real port instead of `Application.get_env(:dala, :liveview_port)`.
...
`dala_new/lib/dala_new/live_view_patcher.ex` (the `dala_live_app_content/4`
function) so newly-generated projects pick up the hash-based default.
Existing projects need a one-line edit in their `dala_app.ex` (or
regenerate).

---

## 5. `mix dala.deploy --native --ios` silently prefers iPhone over sim

**Symptom** — `mix dala.deploy --native --ios` builds and installs on the
...
work or that dala can't see the sim.

**Why** — `dala_dev/lib/dala_dev/native_build.ex` `build_all/1`:
...
`mix dala.devices`. Requires dala_dev ≥ 0.3.18 (earlier versions had a
...
1. **Print what was picked, with the alternative.** Cheapest, lowest
   ...
   ```
   Dala already has `auto_detect_physical_ios` printing a note for the
...
are available), launching a Dala app pops a system dialog:
...
> • lib/arm64-v8a/libdalatest.so : LOAD segment not aligned
...
| `libdalatest.so`         | Dala's NDK build (`android/jni/`) | add to CMakeLists |
...
- **`libdalatest.so`**: add the flag to
...
**Symptom** — `Dala.Test.adjust_slider/4` (and direct `dala_nif:ax_action_at_xy(x, y, :increment)`) returns `:ok` but the slider's value never changes. Verified 2026-04-30 against `dala_test`'s ControlsScreen on a real iPhone with VoiceOver active.
...
**Fix** — in Dala's iOS Slider rendering (the SwiftUI view that backs `<Slider>`), attach an adjustable-action modifier that increments/decrements by a sensible step. A reasonable default is `(max - min) / 10`, configurable via a new optional `step:` prop:
...
This unblocks `Dala.Test.adjust_slider/4` end-to-end. Same component on Android (`dalaSlider` in dalaBridgeKt) should accept a similar step prop and use Compose's `Modifier.semantics { setProgress(...) }`.

**Priority** — medium. The Dala.Test/AX action API exists and works; just needs Dala's components to be conformant.

---

## 8. iOS Toggle's `label:` prop doesn't reach the AX tree

**Symptom** — `Dala.Test.toggle/2` returns `{:error, :label_not_found}` because the visible label text doesn't appear in `dala_nif:ui_tree/0`. The toggle itself comes through as:

```
:button  y=133.5  w=327.0 h=28.0  label=""  value="1"
```

Empty AX label. Pre-existing position-based fallback (`ax_action_at_xy` with toggle index) works, but the natural-feeling `Dala.Test.toggle(node, "Notifications")` doesn't.

**Why** — Dala's iOS Toggle component uses SwiftUI's `Toggle("Label", isOn: $value)` form (or equivalent), but the underlying SwiftUI `Switch` only inherits the label visually — the AX tree exposes it as a separate text node, not as the switch's `accessibilityLabel`. Or more likely the current Dala iOS Toggle renders the label as a sibling Text and the Switch with no `.accessibilityLabel`.

**Fix** — in Dala's iOS Toggle rendering, propagate the `label` prop to `.accessibilityLabel(label)` on the underlying control. Then the toggle appears in AX as `:button label="Notifications" value="1"` and `Dala.Test.toggle(node, "Notifications")` finds it via plain label match. (Android: similar — Compose `Switch` inside `Modifier.semantics { contentDescription = label }`.)

**Priority** — medium. Same as #7 — testing API exists, just needs Dala component to be AX-conformant.

---

## 9. Alert OK-button `accessibilityActivate` is a no-op

**Symptom** — `Dala.Test.dismiss_alert(node, "OK")` (and direct `dala_nif:tap("OK")` while an alert is showing) returns `:ok` but the alert stays on screen.
...
within 2–3 seconds of `dala_start_beam: starting BEAM`.
...
Dala deploy against an Android 16 image — system_image
...
1. **Use Android 14/15 emulators or physical devices.** Older SELinux
   policy permits the cgroup probes. Most existing Dala users are here.
...
We already maintain two OTP-source patches in `dala_dev/scripts/release/patches/`
...
**Symptom** — verified 2026-04-30 against `dala_test` on a Moto E:

```
ui_tree/1                    {:error, :not_loaded}            (dalaBridge.uiTree() not in Kotlin)
...
**Why** — `dala_nif:ui_tree/0` calls a `dalaBridge.uiTree()` static method that
doesn't exist in `dalaBridge.kt` (the C side gracefully returns
`{:error, :not_loaded}` if the symbol's missing). `dala_nif:ax_action/2` and
...
**The right Android implementation is a Compose semantics walker.** Dala's
...
AX actions, `dalaBridge.kt` needs to:
...
**Cost estimate** — ~200 lines Kotlin in `dalaBridge.kt` template (must also
land in `dala_new`'s template so future-generated apps get it), ~50 lines
...
**Where this lands** — WireTap-side concern, but the Dala library hosts the
NIF stubs. Implementation belongs in WireTap (or in `dala_new`'s template if
we want it available to all Dala apps without WireTap).

**Priority** — medium-high if you want `Dala.Test.adjust_slider`/`toggle`/etc.
to work on Android. Low if Dala apps are happy driving via tag-based
`Dala.Test.tap/2` and `Dala.Test.send_message/2`, which work fine on Android
today.

**Related operational note (BeamForegroundService)** — when iterating on
Android NIF/Kotlin changes, `mix dala.deploy --native --device <serial>` does
NOT fully restart the BEAM. Dala's `BeamForegroundService` outlives the
...
adb -s <serial> shell am force-stop com.dala.<app>      # foreground service
...
> 1. **`ios/dala_nif.m`** — the prop parser was reading `text:` for all node
>    types, but app code (and Dala's own demos) pass `value:` for text fields,
>    matching the React/SwiftUI controlled-input convention. Added a
>    `DalaNodeTypeTextField`-gated read of `props[@"value"]` that maps into
>    `node.text`. If both `text:` and `value:` are passed, `value:` wins.
> 2. **`ios/DalaRootView.swift`** — added `.onChange(of: initialText)` to
>    `DalaTextField`. When the parent re-renders with a new value AND the
...
(e.g. via `Dala.Socket.assign(:foo, "100")` from a `handle_info` clause that
...
def handle_info({:tap, {:cycle_product, idx}}, socket) do
  ...
  {:noreply, Dala.Socket.assign(socket, :cart, %{cart | tanks: new_tanks})}
end
...
renderer only reads from it via `@State` / `@Binding`. The Dala renderer
...
> **Resolution.** Wrapped the contents of `DalaTextField`'s
> `ToolbarItemGroup(placement: .keyboard)` in an `if isFocused { ... }`
> guard. Previously every `DalaTextField` in the view tree contributed its
...
**Why (suspected)** — Dala's `:text_field` likely attaches a per-field
...
## 14. iOS sim's distribution node name doesn't match `mix dala.connect`'s expectation

**Symptom** — After `mix dala.deploy --native --ios --device <sim-udid>`,
running `mix dala.connect --no-iex` shows the sim node as a timeout while
...
**Why (suspected)** — `mix dala.connect` constructs the expected sim node
...
`dala_beam.m` derives its node name differently — possibly just
...
sim verification. Use `Dala.Test` against any connected physical device
(which gets the simpler `<app>_ios@<lan-ip>` naming).

**Fix options**

1. **Reconcile the naming.** Either `dala_dev/lib/.../connect.ex` should
   probe both `<app>_ios@127.0.0.1` and `<app>_ios_<udid-prefix>@127.0.0.1`
   in parallel and use whichever responds, or `dala_beam.m` should always
   use the per-UDID-prefix form when running under a sim.

2. **Print the actual registered name from the sim's app log.** Even if
   dala_dev's expectation is wrong, surfacing what the sim is actually
...
agentic-coding loop's foundation; if `Dala.Test` doesn't work against
